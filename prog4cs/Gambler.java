...
public class Gambler {
	...
	public static void main(String[] args) {
		int stake = Integer.parseInt(args[0]); 	// starting with
		int goal = Integer.parseInt(args[1]);	// goal to make
		int trials = Integer.parseInt(args[2]);	// # of trials

		int bets = 0;	// tot # of bets made
		int wins = 0;	// tot # of games won
		...
		for (int t = 0; t < trials; t++) {
			...
			while (cash > 0 && cash < goal) {
				bets++;
				if (Math.random() < 0.7) cash++;	// win
				else	cash--;			// lose
			}
			if (cash == goal) wins++; // won sim?
		}
